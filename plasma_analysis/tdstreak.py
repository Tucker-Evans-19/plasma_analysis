...
def show_image(file, rotation_angle = 0, colormap = 'binary', num_edge_l=-13, num_edge_r=-7):
...
def show_ntd_image(file, colormap = 'binary'):
    filtered_image, shot_num = show_image(file, colormap = 'binary', num_edge_l=-16, num_edge_r = -10)
    return filtered_image, shot_num

# plotting a pxtd image (rotation required to correct image)
def show_pxtd_image(file,rotation_angle = float(.0297*180/3.1415), colormap = 'binary'):
    filtered_image, shot_num = show_image(file, rotation_angle = rotation_angle, colormap = 'binary')
    return filtered_image, shot_num
...
def wiener_decon_lineout(time_axis, lineout, shotnum = 0, noise_left = 0, noise_right = 0, signal_left = 0, signal_right = 0, padding_length = 500):
    ...
    if signal_right == 0 and noise_right ==0: 
        plt.figure()
    ...
    signal_padded = np.hstack((lineout[:signal_right], signal_padding_right))
    signal_time_padded = np.hstack((time_axis[:signal_right], time_padding_right+time_axis[signal_right]))
    ...
    return signal_time_padded, decon
'''    
