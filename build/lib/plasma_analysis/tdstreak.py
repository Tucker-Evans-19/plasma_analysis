...
def show_image(file, rotation_angle = 0, colormap = 'binary'):
    ...
    image_rot = ndimage.rotate(image_clean, rotation_angle)
...
def show_ntd_image(file, colormap = 'binary'):
    filtered_image, shot_num = show_image(file, 'binary')
    return filtered_image, shot_num

# plotting a pxtd image (rotation required to correct image)
def show_pxtd_image(file,rotation_angle = .0297*180/3.1415, colormap = 'binary'):
    filtered_image, shot_num = show_image(file, 'binary')
    return filtered_image, shot_num
...
def image_lineout(file, channel_center, channel_width = 10, plotting = False, shot_num = -1, left_side = -1, right_side = -1, bg_subtract = True, rotation_angle = 0):
    ...
    image_raw = np.array(data.get('Streak_array'), dtype=float)
    image_clean = image_raw[0,:,:] - image_raw[1,:,:]
    image_clean = ndimage.rotate(image_clean, rotation_angle)
    ...
    c_lineout_block = ndimage.median_filter(image_clean[clow:chigh,left_side:right_side], size = (med_filt1, med_filt2))
...
def pxtd_lineout(file, channel_center, channel_width = 10, plotting = False, shot_num = -1, left_side = -1, right_side = -1, bg_subtract = True):
    ...
    lineout = image_lineout(file, channel_center, channel_width = 10, plotting = False, shot_num = -1, left_side = -1, right_side = -1, bg_subtract = True, rotation_angle = .0297*180/3.14159)

    return lineout

def ntd_lineout(file, channel_center, channel_width = 10, plotting = False, shot_num = -1, left_side = -1, right_side = -1, bg_subtract = True):

    lineout = image_lineout(file, channel_center, channel_width = 10, plotting = False, shot_num = -1, left_side = -1, right_side = -1, bg_subtract = True, rotation_angle = 0)
...
def wiener_decon_lineout(time_axis, lineout):
    ...
    print(f'Mean squared noise value: {MSE}') 
    print(f'Estimated signal to noise ratio = {SNR}')
    print(f'Estimaged inverse SNR = {SNR_inv}')
    ...
    signal_freqs, signal_power = ss.periodogram(l2[:1000])
    ...
    plt.savefig(f'power_density_{shotnum}_{channel_num}.png') 
    plt.close()
    ...
    final_value = lineout[final_lineout_index]
    
    # creating the time axis and padding data to the right side of the lineout
    signal_padding_right= final_value*np.exp(-average_time_step*padding_axis_right/1200)
    time_padding_right = padding_axis_right*average_time_step

    # appending the padding data to the right 
    signal = np.hstack((lineout[:final_lineout_index], signal_padding_right))
    signal_time = np.hstack((time_axis[:final_lineout_index], time_padding_right+time_axis[final_lineout_index]))

    # creating the IRF for the PXTD detector
    IRF = pa.pxtd_IRF(np.array(signal_time))
    IRF = nd.gaussian_filter(IRF, 50)
    decon = wiener_decon(signal, IRF, SNR_inv)

    # creating the outputted time trace for the signal:
    output_time = signal_time[:final_lineout_index]
    output_decon = decon[:final_lineout_index]
    
    return output_time, output_decon
